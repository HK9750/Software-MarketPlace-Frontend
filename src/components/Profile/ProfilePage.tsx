...
import { User, MapPin, Phone, Mail, Globe, Edit, Check, X, ShieldCheck, UserCog, ArrowLeft } from "lucide-react"
...
export default function ProfilePage({ userData }: ProfilePageProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    address:
      typeof user.profile.address === "string" ? user.profile.address : JSON.stringify(user.profile.address, null, 2),
  ...
  const handleSaveAll = async () => {
    ...
    try {
      setUpdating(true);
      const changedFields: Partial<MyProfile["profile"] & { websiteLink?: string }> = {};
      ...
      const response = await axios.put<{success: boolean , data: MyProfile}>(
      ...
      <div className="">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-8">
          <Avatar className="h-24 w-24">
            <AvatarImage src="/placeholder.svg" alt={`${user.profile.firstName} ${user.profile.lastName}`} />
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mt-2">
              ...
              <Badge className="bg-primary text-primary-foreground">{user.role}</Badge>
          ...
          <div className="flex gap-2">
            {!editMode ? (
              ...
              <>
                ...
                <Button onClick={handleSaveAll} className="flex items-center gap-2" disabled={!hasChanges()}>
              ...
              <CardContent>
                {!editMode ? (
                  <div className="flex items-start gap-2">
                    ...
                    <span>{formatAddress(user.profile.address)}</span>
                  </div>
                ) : (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      You can enter a simple address string or a JSON object with fields like street, city, state, etc.
                    </p>
                  </div>
                )}
              </CardContent>
                  ...
                  {!editMode ? (
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {user.sellerProfile?.websiteLink || "Not provided"}
                        </a>
                      </div>
                    </div>
                  ) : (
