...
export default function ProductCatalog({ products }: ProductCatalogProps) {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    if (selectedPriceRange) {
      ...
      if (selectedRange && (numericPrice < selectedRange.min || numericPrice > selectedRange.max)) {
        return false
      }
    }
        ...
        <div className="flex gap-2 ">
          <Sheet>
            <SheetTrigger asChild>
              <Button id="wishlist-trigger" variant="outline" size="sm" className="flex items-center gap-1">
                <Heart className={cn("h-4 w-4", wishlist.length > 0 ? "fill-red-500 text-red-500" : "")} />(
                {wishlist.length})
              </Button>
            </SheetTrigger>
        ...
        <div className="space-y-6">
          ...
          {filteredProducts.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mt-2">Try adjusting your filters to find what you are looking for</p>
